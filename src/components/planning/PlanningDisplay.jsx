...
import { saveToLocalStorage, loadFromLocalStorage, saveShopBackup, loadShopBackup } from '../../utils/localStorage';
...
const PlanningDisplay = ({ config, selectedShop, selectedWeek, selectedEmployees, planning: initialPlanning, onBack, onBackToShop, onBackToWeek, onBackToConfig, onReset, setStep, setGlobalPlanning, setFeedback }) => {
  ...
  const [planning, setPlanning] = useState(loadShopBackup(selectedShop).weeks[selectedWeek]?.planning || initialPlanning || {});
  ...
  const [availableWeeks, setAvailableWeeks] = useState(
    Object.keys(loadShopBackup(selectedShop).weeks || {}).map(weekKey => ({
      key: weekKey,
      date: new Date(weekKey),
      display: `Semaine du ${format(new Date(weekKey), 'd MMMM yyyy', { locale: fr })}`
  ...
  const shops = loadFromLocalStorage('shops', [selectedShop]);
  ...
  useEffect(() => {
    ...
    const storedEmployees = shopData.weeks[currentWeek]?.selectedEmployees || selectedEmployees || [];
  ...
  useEffect(() => {
    if (Object.keys(planning).length && config?.timeSlots?.length) {
      console.log('Saving planning to localStorage:', `backup_${currentShop}`, planning);
      const shopData = loadShopBackup(currentShop);
      shopData.weeks[currentWeek] = shopData.weeks[currentWeek] || {};
      shopData.weeks[currentWeek].planning = planning;
      shopData.weeks[currentWeek].selectedEmployees = selectedEmployees;
      saveShopBackup(currentShop, shopData);
      setGlobalPlanning(planning);
      setAvailableWeeks(Object.keys(shopData.weeks).map(weekKey => ({
        key: weekKey,
        date: new Date(weekKey),
        display: `Semaine du ${format(new Date(weekKey), 'd MMMM yyyy', { locale: fr })}`
      })).sort((a, b) => a.date - b.date));
  ...
  }, [planning, currentShop, currentWeek, config, selectedEmployees, setGlobalPlanning]);
  ...
  const toggleSlot = useCallback((employee, slotIndex, dayIndex, forceValue = null) => {
    ...
    setPlanning(prev => {
      ...
      const shopData = loadShopBackup(currentShop);
      shopData.weeks[currentWeek] = shopData.weeks[currentWeek] || {};
      shopData.weeks[currentWeek].planning = updatedPlanning;
      saveShopBackup(currentShop, shopData);
      setGlobalPlanning(updatedPlanning);
      return updatedPlanning;
    });
  }, [config, currentWeek, currentShop, setGlobalPlanning]);

  const changeWeek = (direction) => {
    const newWeek = addDays(new Date(currentWeek), direction * 7);
    setCurrentWeek(newWeek.toISOString());
    const shopData = loadShopBackup(currentShop);
    setPlanning(shopData.weeks[format(newWeek, 'yyyy-MM-dd')]?.planning || {});
    setLocalFeedback('');
  };
  ...
  const changeShop = (newShop) => {
    ...
    setPlanning(shopData.weeks[format(new Date(currentWeek), 'yyyy-MM-dd')]?.planning || {});
    setLocalFeedback('');
  };
  ...
  return (
    <div className="planning-container">
      ...
      <div className="recap-buttons" style={{ display: 'flex', flexDirection: 'row', overflowX: 'auto', justifyContent: 'center', gap: '12px', marginBottom: '15px' }}>
        {(loadShopBackup(currentShop).weeks[currentWeek]?.selectedEmployees || selectedEmployees || []).map((employee, index) => (
          <div
            key={employee}
            style={{
              display: 'flex',
      ...
      <PlanningTable
        ...
        planning={planning}
        selectedEmployees={loadShopBackup(currentShop).weeks[currentWeek]?.selectedEmployees || selectedEmployees || []}
